...
check_storage() {
  ...
  if [ "$available_storage" -lt 250 ] && [ ! -d "$rom_dir"/ ] && [ ! -d /lineage ] && [ ! -d /pixelos ]; then
...
repo_init() {
  if [ ! -d "$rom_dir"/ ] && [ ! -d "$rom_dir"/.repo/ ]; then
    rm -rf /lineage /pixelos
    mkdir "$rom_dir"/
    cd "$rom_dir"/
    repo init -u https://github.com/crdroidandroid/android.git -b "$android_ver" --git-lfs --depth=1  | tee /tmp/android-sync.log
...
upload_rom() {
  ...
  for device in "${devices[@]}"; do
    cp "$out_dir"/target/product/"$device"/crDroidAndroid-"$android_ver"-"$build_date"-"$device"-*.zip "$ota_dir"/
