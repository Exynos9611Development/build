...
repo_init() {
  if [ ! -d /crdroid ]; then
    mkdir /crdroid
    cd /crdroid
    repo init -u https://github.com/crdroidandroid/android.git -b 15 --git-lfs --depth=1  | tee /tmp/android-sync.log
...
upload_rom() {
  local tag_name="CRD-$build_date" 
  git clone https://github.com/Exynos9611Development/OTA OTA
  for device in "${devices[@]}"; do
    cp out/target/product/"$device"/crdroid*-"$build_date"-UNOFFICIAL-"$device".zip OTA/
